...
class ScansDataLoader(Dataset):
    ...
    def __getitem__(self, idx: int) -> Tuple[np.ndarray, np.ndarray, str]:
        ...
        return points, labels, f"{self.scan_id[scan_idx]}"
...
if __name__ == '__main__':
    ...
    FRAMES_DIR = '/home/ibrahim/Documents/AnyDesk'
    frames = os.listdir(FRAMES_DIR)

    wdl = ScansDataLoader(FRAMES_DIR, frames)

    print(len(wdl))

    for i in range(0, len(wdl)):
        data, _, id = wdl[i]
        np.savetxt(os.path.join(FRAMES_DIR,id+'.asc'), data, fmt='%.3e')
